...
public class dbQuePasa extends SQLiteOpenHelper {
    ...
    private final String CREATE_TABLE_USER = "create table " + TAB_USER + "("+ COL_USER_ID + " integer primary key, "+
            COL_USER_NAME + " text not null, "+ COL_USER_EMAIL + " text, "+ COL_USER_IMG + " text)";
    ...
    private final String CREATE_TABLE_MSG = "create table " + TAB_MSG +"("+ COL_MSG_ID + " integer primary key, "+
            COL_MSG_MSG + " text not null, "+ COL_MSG_DATE + " text not null, "+ COL_MSG_ID_USER + " integer not null, "+
            COL_MSG_LAST_MSG + " integer default 0 not null, "+ "FOREIGN KEY ("+ COL_MSG_ID_USER +") REFERENCES " + TAB_USER + "(" + COL_USER_ID + "))";
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        Log.w(dbQuePasa.class.getName(),"Upgrading database from version " + oldVersion + " to "+ newVersion + ", which will destroy all old data");
